...
- fix(healthcheck): three bugs found by running `dark-factory healthcheck` against a real `.dark-factory.yaml` project — the healthcheck found its own defects. (1) **Project-name fallback:** `CreateHealthcheckCommand` was wiring `cfg.ResolvedProjectOverride()` directly into `BootContainerProbe.ProjectName` and `NewClaudeProbe`'s `projectName` arg, which returns `""` when neither `project:` nor `projectName:` is set; the resulting container names started with `-` and docker rejected them with `exit status 125`. Now uses `project.Resolve(cfg.ResolvedProjectOverride()).String()` (git-root fallback), matching every other factory call site. (2) **Boot probe `docker run` design:** the spec described a three-step "docker run (detached) → WaitUntilRunning → docker exec sh -c probe" sequence, but the implementation ran `docker run` in **foreground** with no `-d` flag, which blocks until the container exits. With `claude-yolo`'s default entrypoint invoked with no args, the container exits non-zero and the wait/exec steps never run. Collapsed into a single synchronous `docker run --rm --entrypoint /bin/sh <image> -c '<probe>'` call. `BootContainerProbe.ContainerChecker` field removed (no longer used); `runProbe` replaces `startContainer` + `waitForRunning` + `verifyBoot`. (3) **Mount probe missing `--entrypoint /bin/sh` and the workspace mount itself:** `mountProbe.Run` ran `docker run --rm <image> sh -c <cmd>` without `--entrypoint`, so the image's claude entrypoint consumed `sh -c <cmd>` as args and exited 4; and the probe never bound `<workspaceDir>:/workspace`, so even with `sh` it would have written to an in-container overlay rather than testing the actual host mount. Both fixed. The `claude` probe (`NewClaudeProbe`) is partially patched — `--entrypoint claude` bypasses the image-default entrypoint issue — but it still lacks the `<claudeDir>:/home/node/.claude` mount + `ANTHROPIC_*` env-var passthrough that a real prompt container gets, so it currently fails on a real install with `exit status 1`. Workaround: run `dark-factory healthcheck --no-claude` until follow-up wiring lands. Tests under `pkg/runner/probe_test.go` and `pkg/cmd/healthcheck/probes_test.go` rewritten to match the single-call probe shape.
