...
- feat: add a `generating-without-prompt` detector to `dark-factory doctor` — reports specs parked in `specs/in-progress/` with `status: generating` and no prompt referencing them anywhere; `generating` is transient and nothing times it out, so a generation that dies (container OOM-killed, host rebooted, prompts approved out from under a running generator) leaves the spec stuck forever with nothing on disk, and no existing detector covers it — `verifying-stale` keys on `verifying`, `prompted-but-not-swept` on `prompted`, and `status-dir-mismatch` explicitly permits `generating` in that directory. A spec that *does* have prompts is deliberately not reported even while still marked `generating`: that is bookkeeping drift with the work visible on disk, not a failed generation
...
- chore(deps): bump default YOLO container image `docker.io/bborbe/claude-yolo` `v0.15.3` → `v0.16.0` (Claude Code `2.1.233` → `2.1.270`). Smoke-tested via `dark-factory healthcheck` (all probes pass, incl. headless `claude`) against the new image.

## v0.195.3

- docs: prompt-writing guide now covers the two exit-code traps in `<verification>` blocks — `grep -c` exits 1 on a zero count (use `! grep -q` for absence assertions), and a piped command reports only its last stage (use `set -o pipefail`).
...
- chore(deps): bump default YOLO container image `docker.io/bborbe/claude-yolo` `v0.14.1` → `v0.15.0` (Claude Code `2.1.219` → `2.1.233`). Smoke-tested via `dark-factory healthcheck` (all probes pass, incl. headless `claude`) against the new image.
...
- fix: `specsweeper.Sweep` now checks `ctx.Done()` between iterations. It runs on every daemon tick and does I/O per spec, so a cancelled context previously let the loop run to completion — blocking shutdown until the whole spec set had been walked.
...
- docs(scenario-execution): correct time budgets measured on a full 12-scenario walk (2026-07-24, claude-yolo v0.14.0). Scenario 019 was listed at ≈2.5 min but its generate+audit phase alone runs ~6 min; it now has its own row (up to ~8 min) plus an explicit warning that a wait loop expiring as `auto-approve: approved generated prompt` lands leaves the prompt queued-but-unexecuted — indistinguishable from the historical "executor never picked up the prompt" bug, but resolved by re-running `dark-factory run` in the same sandbox. Full-gate estimate raised from ~30 min / ~$0.60 to ~45–60 min / ~$1.
- docs(scenario-execution): add scenario 024 (backend:local fails closed) to the per-scenario reference — it was missing entirely — and add two pitfall rows: the zsh function-local `trap ... EXIT` that deleted the sandbox on Setup return (`fatal: Unable to read current working directory`), and the stale-assertion class where a scenario greps for wording the binary no longer emits, reporting FAIL on provably correct behavior.
...
- test: add scenario 024 validating `backend: local` end-to-end — resolves from config, reaches the backend, fails closed via localSubprocessExecutor when `claude` is absent, and spawns no docker container / contacts no docker daemon (spec 104 AC 4/6/9)
...
## v0.191.1
- docs(scenario-019): add `## Provider Dependency` section documenting that the auto-approve happy-path scenario is provider-agnostic and passes on both Anthropic and proxy-routed MiniMax (verified v0.191.0); records the resolved v0.187.11 minimax executor-pickup failure and its spec-101 root cause.
...
- feat(prompt-creator): mirror the two rules at authoring time. Constraints added: NEVER emit bare `git ` in `<verification>` when target repo has `hideGit: true` OR `workflow: worktree`; NEVER emit operator-only commands in `<verification>`; NEVER add a "NOTE FOR OPERATOR" / operator-vs-agent step split inside a prompt (artifact-type-mismatch tell). Together these prevent the failure-mode class at authoring time and catch it at review time.
- docs(spec-writing): add "Verification: container-executable vs operator-executable" section. Guides spec authors to split the `# Verification` block into two named rungs — container-executable (prompt time) and operator-executable (spec verification ladder, post-merge). Prevents operator-only commands from leaking into prompts as `NOTE FOR OPERATOR` blocks.
- docs(prompt-writing): add "`<verification>` is container-only — never operator-side" section with concrete allow-list and deny-list of command families, explanation of the false-positive-pass failure mode (executor does not check verification exit codes), and pointer to the spec-writing operator-executable rung.
...
- chore(deps): bump default claude-yolo container image from `v0.11.1` to `v0.12.0`. The new image honours `ANTHROPIC_BASE_URL`, so dark-factory containers can be routed through a local `claude-code-router` instance to serve non-Anthropic models (MiniMax, DeepSeek, GLM, …) without code changes. Updates `pkg.DefaultContainerImage` (the single source of truth); historical version strings inside `specs/completed/`, `prompts/completed/`, and the version-pinned fixtures in `pkg/launchpolicy/policy_test.go` are intentionally untouched. (Note: this bullet was originally misfiled under `## v0.188.0` due to a CHANGELOG conflict-resolution race during rebase — the v0.188.0 git tag does NOT contain the image bump. Moved to `## Unreleased` so the release-watcher cuts the next version with the code that actually exists in the tag.)
- docs(configuration): add "Routing via claude-code-router" subsection under Global env with a MiniMax-M2.7 example config (`model: MiniMax-M2.7-highspeed` + `ANTHROPIC_BASE_URL: http://host.docker.internal:8788`) and notes on `host.docker.internal` resolution, model passthrough, and skipping `ANTHROPIC_API_KEY` when the router handles upstream auth.
...
- fix: preflight subprocess runner no longer inherits the general-purpose `subproc.DefaultTimeout` of 10 seconds. Spec 100 (Centralize Subprocess Runner) migrated `pkg/preflight/preflight.go:runInContainer` from raw `exec.CommandContext` (no timeout — only the daemon's parent ctx bound it) to `subprocRunner.RunWithWarnAndTimeoutDir(...)` (10s hard timeout). For bborbe Go repos a cold-cache `make precommit` takes 30–180s (tests + lint + security scans), so every non-trivial preflight failed silently after the 10s mark and operators worked around it with `preflightCommand: "true"` until the regression was diagnosed. Factory now constructs preflight's `subproc.Runner` via a dedicated `newPreflightSubprocRunner()` helper with `(warnAfter=30s, timeout=30min)`. (The `v0.187.10` git tag was cut from PR #52 only; this bullet was misfiled under `## v0.187.10` due to a CHANGELOG conflict-resolution race when PR #53 merged — the v0.187.10 tag does NOT contain this fix. Moving it to `## Unreleased` so the release-watcher cuts v0.187.11 with the code that actually exists in the tag.)

## v0.187.10

- chore(deps): bump default claude-yolo container image from `v0.11.0` to `v0.11.1`. The new image pins `@anthropic-ai/claude-code` to `2.1.169` (claude-yolo previously tracked `latest`, which silently inherited an upstream marketplace-consent regression that broke headless `claude -p` plugin discovery — dark-factory spec generation died with `Unknown command: /dark-factory:generate-prompts-for-spec`). Bump is reproducible: rebuilding `v0.11.1` always produces the same claude version. Updates `pkg.DefaultContainerImage` (the single source of truth) plus the v0.11.0 references in `README.md`, `docs/configuration.md`, `docs/init-project.md`, and `docs/yolo-container-setup.md`. Historical version strings inside `specs/completed/` and `prompts/completed/` and the version-pinned fixtures in `pkg/launchpolicy/policy_test.go` are intentionally untouched.
...
- refactor: promote `pkg/factory.resolveHideGit` to `pkg/config.Config.EffectiveHideGit()` (matches the established `EffectiveMaxContainers` / `HealthcheckEnabledValue` pattern). The helper had existed in factory but was applied at 1 of 3 sites (factory.go:660 used it; lines 988 and 1073 inlined the formula `cfg.Workflow == config.WorkflowWorktree || cfg.HideGit`). Same drift class that caused the 2026-06-26 silent minimax healthcheck failure — fixed structurally now. Adds `(p ProcessorConfig) EffectiveHideGit()` so the formula is co-located with both data shapes. Adds `scripts/hotpath-hidegit-check.sh` precommit gate that rejects the inline expansion in pkg/ outside the canonical methods (allowlist: `pkg/config/config.go`, `pkg/factory/factory.go`, `pkg/config/loader.go`). Phase-2 follow-up of [[Harden Dark Factory Architecture]] from the 2026-06-27 architecture re-audit.
...
- fix: heal `dark-factory healthcheck` for alt-provider configs (minimax, self-hosted vLLM). Three compounded regressions, fixed together so the probe runs the SAME claude argv as production prompts:
  ...
  - **Claude probe stops overriding `--entrypoint claude`**: the image's `/usr/local/bin/entrypoint.sh` does setpriv (root → node), proxy/firewall init, and supplies `--dangerously-skip-permissions` + `--model` + `--output-format stream-json` for production prompts. The probe now exercises that exact path via `YOLO_PROMPT` + `YOLO_OUTPUT=json` env (matching `pkg/executor/executor.go:464-466`), so any regression in production claude argv surfaces in the probe by construction. Regression introduced by commit c237af6 (2026-06-16) "share docker-launch path with production" which kept the override.
  - **Healthcheck factory always forces `hideGit=true`**: probes don't read the project's `.git` dir, and the image entrypoint.sh runs `git config --global` which exit-128s in a worktree (broken `.git` pointer). Production `dark-factory run` exposes `--set hideGit=true`; healthcheck has no equivalent surface, so hard-force.
- All three fixes are required together — the env merge alone leaves the probe silent on alt-provider configs; the entrypoint alignment alone leaves env empty; the hideGit fix alone leaves either preceding issue uncaught from a worktree.
