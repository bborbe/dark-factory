...
3. **Sibling entry points covered?** For Go projects, ran `grep -rn "factory.Create\|func.*Run(ctx" <package>/` and confirmed all parallel `application.Run` methods + factory call sites are updated together — OR explicitly noted out-of-scope in the prompt.
