...
Run this as a final sanity check after reading all ACs. Write a 2-3 sentence "laziness pass" subsection in the report:

> **Adversarial laziness pass**: laziest implementation that passes every AC = [description]. [PASS — the laziest impl is non-trivial enough] / [FAIL — under-specified ACs: N, M].

If FAIL, suggest concrete tightening for each under-specified AC.
...
Flag pattern:
> Line 47: "the agent should retry **appropriately**" — UNRESOLVED hedge. Specify retry count and backoff, or mark as "agent decides at impl time" if the decision is local.
