...
**Before writing any `<verification>` command, check it against the catalogued failure mechanisms** — the Personal vault's `50 Knowledge Base/Verification Commands That Cannot Fail.md` lists 22 of them, each observed and confirmed empirically. The recurring ones are `grep -c` exiting non-zero when the count is zero (absence assertions must be `! grep -q`); a line-scoped `grep -A<n>` window that swallows a neighbouring section (`grep -A20 '^## Unreleased' | grep -c '^- fix:'` returns 3 on a correct file — use the section-walking `awk '/^## /{sec=$0} …'` form); grepping the output of a command that never ran; a threshold the pre-existing state already meets; and grepping a string the tool does not print (e.g. `go tool cover -func` emits bare method names, never receiver types). Every one is invisible to reading and obvious to running — and each has cost a rework cycle, because the auditor catches them only after generation.
...
**Never write `-mod=vendor` in a verification command.** bborbe repos don't commit `vendor/`, so `go test -mod=vendor …` (and `go tool cover` on its profile) fails with "inconsistent vendoring". Always use `-mod=mod` for `go test` / coverage verification commands.
