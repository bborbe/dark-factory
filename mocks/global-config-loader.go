// Code generated by counterfeiter. DO NOT EDIT.
package mocks

import (
	"context"
	"sync"

	"github.com/bborbe/dark-factory/pkg/globalconfig"
)

type GlobalConfigLoader struct {
	LoadStub        func(context.Context) (globalconfig.GlobalConfig, error)
	loadMutex       sync.RWMutex
	loadArgsForCall []struct {
		arg1 context.Context
	}
	loadReturns struct {
		result1 globalconfig.GlobalConfig
		result2 error
	}
	loadReturnsOnCall map[int]struct {
		result1 globalconfig.GlobalConfig
		result2 error
	}
	invocations      map[string][][]interface{}
	invocationsMutex sync.RWMutex
}

func (fake *GlobalConfigLoader) Load(arg1 context.Context) (globalconfig.GlobalConfig, error) {
	fake.loadMutex.Lock()
	ret, specificReturn := fake.loadReturnsOnCall[len(fake.loadArgsForCall)]
	fake.loadArgsForCall = append(fake.loadArgsForCall, struct {
		arg1 context.Context
	}{arg1})
	stub := fake.LoadStub
	fakeReturns := fake.loadReturns
	fake.recordInvocation("Load", []interface{}{arg1})
	fake.loadMutex.Unlock()
	if stub != nil {
		return stub(arg1)
	}
	if specificReturn {
		return ret.result1, ret.result2
	}
	return fakeReturns.result1, fakeReturns.result2
}

func (fake *GlobalConfigLoader) LoadCallCount() int {
	fake.loadMutex.RLock()
	defer fake.loadMutex.RUnlock()
	return len(fake.loadArgsForCall)
}

func (fake *GlobalConfigLoader) LoadCalls(stub func(context.Context) (globalconfig.GlobalConfig, error)) {
	fake.loadMutex.Lock()
	defer fake.loadMutex.Unlock()
	fake.LoadStub = stub
}

func (fake *GlobalConfigLoader) LoadArgsForCall(i int) context.Context {
	fake.loadMutex.RLock()
	defer fake.loadMutex.RUnlock()
	argsForCall := fake.loadArgsForCall[i]
	return argsForCall.arg1
}

func (fake *GlobalConfigLoader) LoadReturns(result1 globalconfig.GlobalConfig, result2 error) {
	fake.loadMutex.Lock()
	defer fake.loadMutex.Unlock()
	fake.LoadStub = nil
	fake.loadReturns = struct {
		result1 globalconfig.GlobalConfig
		result2 error
	}{result1, result2}
}

func (fake *GlobalConfigLoader) LoadReturnsOnCall(i int, result1 globalconfig.GlobalConfig, result2 error) {
	fake.loadMutex.Lock()
	defer fake.loadMutex.Unlock()
	fake.LoadStub = nil
	if fake.loadReturnsOnCall == nil {
		fake.loadReturnsOnCall = make(map[int]struct {
			result1 globalconfig.GlobalConfig
			result2 error
		})
	}
	fake.loadReturnsOnCall[i] = struct {
		result1 globalconfig.GlobalConfig
		result2 error
	}{result1, result2}
}

func (fake *GlobalConfigLoader) Invocations() map[string][][]interface{} {
	fake.invocationsMutex.RLock()
	defer fake.invocationsMutex.RUnlock()
	copiedInvocations := map[string][][]interface{}{}
	for key, value := range fake.invocations {
		copiedInvocations[key] = value
	}
	return copiedInvocations
}

func (fake *GlobalConfigLoader) recordInvocation(key string, args []interface{}) {
	fake.invocationsMutex.Lock()
	defer fake.invocationsMutex.Unlock()
	if fake.invocations == nil {
		fake.invocations = map[string][][]interface{}{}
	}
	if fake.invocations[key] == nil {
		fake.invocations[key] = [][]interface{}{}
	}
	fake.invocations[key] = append(fake.invocations[key], args)
}

var _ globalconfig.Loader = new(GlobalConfigLoader)
