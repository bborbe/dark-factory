...
var _ = Describe("spec-098 regression lock: launch policy is the single source of truth", func() {
	...
	It(
		"propagates security fields through Policy.WithSecurity to executor.BuildDockerRunArgs",
		func() {
			...
			args := executor.BuildDockerRunArgs(
				p.BuildOpts(launchpolicy.Extras{ContainerName: "x"}),
			)
