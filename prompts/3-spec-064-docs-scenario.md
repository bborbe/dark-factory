...
- `dark-factory --help` (top-level) references the four formats in the `approve` and `complete` command lines
- New scenario `scenarios/017-spec-064-flexible-id-matching.md` walks all four format resolutions, the ambiguity case, and cwd walk-up (success and failure) against a freshly built binary
- No Go source code is changed in this prompt — only help strings in `main.go` and the new scenario file
</summary>
...
```bash
rm -rf "$WORK_DIR" "$AMBIG_DIR" /tmp/df-no-project-test
```
```
