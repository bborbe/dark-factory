...
queued: "2026-04-25T19:21:19Z"
started: "2026-04-25T19:21:21Z"
---
...
<objective>
Reduce reading friction and structural smell across the repo by splitting oversized Go files into focused per-concern files. Currently three test files exceed 2,000 lines, the largest at 7,450 lines, which causes Read-tool failures, slow IDE navigation, and frequent merge conflicts. After this prompt, no hand-written Go file in the repo exceeds 2,000 lines, and the next prompt (`add-file-length-linter`) will enable a revive `file-length-limit` rule to prevent regressions.
</objective>
...
The "File size" rule (from the dark-factory YOLO CLAUDE.md) defines these thresholds:
...
Known offenders post-348 (verify with the survey — line counts may have shifted further):

| File | Lines | Notes |
|------|-------|-------|
| `pkg/processor/processor_test.go` | 7,450 | grew during refactor (groups 1-5) — many counterfeiter mocks added |
| `pkg/prompt/prompt_test.go` | 3,067 | not touched by refactor |
| `pkg/config/config_test.go` | 2,648 | not touched by refactor |

These three are the primary targets. `processor_internal_test.go` (was 2,934) shrank to 1,590 during the refactor — already under the limit, exclude it.

If the survey reveals additional files > 2,000 lines (e.g. a new file shipped between drafting and execution), include them.
...
Concerns for `processor_test.go` (post-refactor — most logic now in extracted packages, so processor tests focus on orchestration and the injected services):

| Concern | Suggested filename |
|---------|-------------------|
| `Process()` event-loop orchestration (queue tick, sweep tick, ready signal, onIdle callback) | `processor_loop_test.go` |
| Per-prompt processing (`ProcessPrompt`, `runContainer`, cancellation watcher integration) | `processor_runprompt_test.go` |
| Resume-on-startup (`ResumeExecuting`, `ResumeCommitting`) | `processor_resume_test.go` |
| Constructor wiring + service-injection-shape tests | `processor_constructor_test.go` |
| Shared `newTestProcessor` helper, BeforeEach setup, fakes | `processor_helpers_test.go` |

Aim: each output file 500–1,500 lines. If a single concern exceeds 1,500 lines on its own, split it further (e.g. `processor_loop_queue_test.go`, `processor_loop_sweep_test.go`).

For `prompt_test.go`, split along `Frontmatter` / `PromptFile` / `Manager` / `ListQueued` lines.
...
If anything fails (compile error, missing import, missing helper), fix immediately before moving on. Do not batch all three splits then run tests at the end — debugging a multi-file failure is much harder.

**Behaviour-preservation sanity check:** before splitting each file, capture the spec count:

```bash
go test -v -count=1 ./pkg/processor/... 2>&1 | grep -c "^=== RUN"
```

After all splits land, run the same command on the same package and assert the count is identical. Ginkgo's `var _ = Describe` registrations are package-global so file boundaries should not change discovery — this command makes the invariant explicit.
...
The `&& $2 != "total"` filter is required — `wc -l` adds a `total` summary line whose count is the sum of all files (always > 2000 across the repo). Without the filter the survey reports a false positive.
...
```
- refactor: split oversized Go test files (`processor_test.go` 7450→<1500/file, `prompt_test.go` 3067→<1500/file, `config_test.go` 2648→<1500/file) into per-concern files; no behavior changes, all tests pass; eliminates Read-tool token-limit friction and reduces merge-conflict surface
```

Adjust the file list to match what was actually split. Append to `## Unreleased` if it already exists; do NOT replace existing entries.
...
- Helper file naming: prefer `<pkg>_helpers_test.go` (e.g. `processor_helpers_test.go`). If the project already has a different convention in use, match that — `ls pkg/<name>/*_test.go` to confirm before committing to a name.
...
# Three known offenders shrunk (processor_internal_test.go was already under-limit post-refactor)
wc -l pkg/processor/processor_test.go pkg/prompt/prompt_test.go pkg/config/config_test.go
