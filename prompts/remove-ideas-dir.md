...
2. Remove `ideasDir` field from `pkg/status/status.go` `statusCollector` struct. Remove `IdeasCount` from `Status` struct. Remove the ideas counting logic in `Get()` method.
...
5. Update `NewStatusCollector` constructor signature — remove `ideasDir` parameter. Update all callers in `pkg/factory/factory.go`.
</requirements>
