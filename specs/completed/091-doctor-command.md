...
- Six detection categories are in scope, drawn from the real failure shapes seen in operation. Anything broader (auto-retry, cross-project, schema migrations, **`failed-but-merged` prompt detection** — needs a `commit` field that doesn't exist in `prompt.Frontmatter` today; deferred to a follow-up spec) is explicitly out of scope.
...
- Do NOT add new detection categories beyond the six below in this spec. Additional detectors are follow-up specs.
- Do NOT implement a `failed-but-merged` detector here. The category requires a `commit` SHA field in `prompt.Frontmatter` that does not exist today; adding the field is a separate spec, after which a follow-up spec can add this detector.
- The seven-category framing in earlier drafts of this spec mentioned `failed-but-merged` — that category is removed here and deferred. References to it in DB / AC have been pruned.
...
6. Detection category — **prompt links non-existent spec**: a prompt file frontmatter `spec: [<id>]` references a spec id that has no corresponding file in any `specs/<lifecycle-dir>/`. Each finding names the prompt file and the missing spec id. Fix-command line: `dark-factory prompt unlink <prompt-id>` OR `dark-factory prompt relink <prompt-id> <new-spec-id>`. The CLI prints **one** finding per orphan with **both** alternatives in a single Fix block (operator chooses); the `--fix` path defaults to `unlink` and requires the operator to explicitly pass `--relink=<id>` to take the other branch.
7. Detection category — **orphan in-progress prompts**: a prompt file lives in `prompts/in-progress/` AND its parent spec has `status: completed` or `status: rejected`. Fix-command line: `dark-factory prompt cancel <prompt-id>`. (`prompt complete` was considered but is unsafe post-move; `--fix` defaults to `cancel` and never auto-completes — operator can override manually.)
8. Detection category — **status contradicts directory**: a spec or prompt file's frontmatter `status` value does not match the lifecycle directory it lives in (e.g. `status: completed` in `specs/in-progress/`, or `status: draft` in `specs/in-progress/`). Fix-command line: `dark-factory spec move <spec-id>` (or the prompt equivalent).
9. Running `dark-factory doctor --fix` interactively prompts `Apply? [y/N]` per finding before any mutation; with `--yes`, all confirmations auto-accept. Each applied fix appends one line to `.dark-factory/doctor.log` containing timestamp (RFC3339), finding category, target file path(s), action taken, before-state, after-state.
10. When `doctor --fix` renumbers a spec, the new spec file's frontmatter records `previous_id: NNN` so the rename is traceable. The YAML marshaller emits the value **unquoted** (e.g. `previous_id: 056\n`, not `previous_id: "056"\n`). Linked prompt files' `spec:` frontmatter is rewritten to the new id; their filenames are NOT touched.
11. The daemon's existing startup reconciliation path is **removed entirely**. The silent renumber code path (the call site(s) under `pkg/processor/`, `pkg/factory/`, `pkg/specwatcher/`, `pkg/cmd/`, `main.go` that today rename spec files during startup) is deleted, not demoted to log. Any detection-from-daemon is out of scope for this spec — operators run `dark-factory doctor` on demand. Demote-to-log was considered and rejected: it keeps dead code on the silent-mutation surface and creates a permanent grep-allowlist entry; if scheduled in-daemon detection is wanted later, that is a separate spec running `doctor` on a timer, not a partial revival of the renumber path.
12. `dark-factory doctor` and `dark-factory doctor --fix` are read-only with respect to `specs/completed/`, `specs/rejected/`, `prompts/completed/`, `prompts/cancelled/`. These directories are reported on but never written.
...
- The project layout (`specs/`, `specs/in-progress/`, `specs/completed/`, `specs/rejected/`, `prompts/`, `prompts/in-progress/`, `prompts/completed/`, `prompts/cancelled/`) is frozen — `doctor` reads from but does not introduce new lifecycle directories. (Earlier drafts of this spec listed `specs/ideas/`; that directory does not exist in `pkg/config/config.go:SpecsConfig` and is dropped.)
...
- [ ] `dark-factory doctor --fix --yes` against the duplicate-number fixture writes the renumber: the second spec file is renamed to use the next free number, its frontmatter records `previous_id: 056` **unquoted** (`previous_id: 056\n`, not `previous_id: "056"\n`), every linked prompt's `spec:` frontmatter is rewritten to the new id, and `.dark-factory/doctor.log` gains one audit line per action — evidence: `grep -c '^previous_id: 056$' specs/in-progress/057-*.md` returns 1 (anchored end-of-line forbids the quoted form) AND `grep -c 'spec: \[057\]' prompts/in-progress/*spec-056*.md` plus `prompts/completed/*spec-056*.md` accounts for every prior `spec: [056]` linkage AND `wc -l .dark-factory/doctor.log` returns ≥1.
...
grep -rn 'Reindex\|RenumberSpecsAfterRemoval' pkg/processor/ pkg/factory/ pkg/specwatcher/ pkg/cmd/ main.go
```
