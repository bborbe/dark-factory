...
- Different ergonomics from `autoGeneratePrompts: false` (the default): that setting pauses automation; this subcommand replaces it with an explicit invocation.
...
- Breaking change risk if auto-trigger is removed entirely; could ship alongside `autoGeneratePrompts: false` (the default) so operators choose policy vs. explicit-only.
