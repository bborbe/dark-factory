...
2. When `autoRetry` is enabled and a prompt fails (timeout or any other reason), the daemon increments `retryCount` in the prompt frontmatter and re-queues the prompt for execution, up to `autoRetryLimit` times.
...
- When `autoRetry` is disabled, failed prompts behave as they do today (prompt marked `failed`, daemon behavior unchanged).
- Config defaults allow zero-config upgrade: existing projects get 90-minute timeout with 3 auto-retries without changing their config.
...
| `autoRetryLimit: 0` | No auto-retries, fail immediately (same as `autoRetry: false`) | Intentional |
...
- [ ] Failed prompt is auto-retried up to `autoRetryLimit` times when `autoRetry: true`
...
- [ ] Missing config fields default to: `maxPromptDuration: 90m`, `autoRetry: true`, `autoRetryLimit: 3`
