...
4. If the work commit fails (e.g., precommit hook, branch protection, push rejection), the daemon ROLLS THE PROMPT FILE BACK to `prompts/in-progress/` with its original frontmatter (`status: in_progress`). No orphan `completed/` file is left on disk. This applies uniformly across all four workflow modes.
...
|---------|-------------------|-----------|---------------|----------|
| Work commit fails after move (precommit hook rejects) | File rolled back to `in-progress/` with `status: in_progress` restored | non-zero exit from `git commit`; log line `move-rolled-back-after-commit-failure` with the failing command output | reversible | Operator fixes the precommit cause; daemon re-runs the prompt, producing a single combined commit |
