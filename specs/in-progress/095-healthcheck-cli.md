...
- [ ] `CHANGELOG.md` has a new `## Unreleased` entry referencing `healthcheck` — evidence: `grep -nE '^- .*healthcheck' CHANGELOG.md` returns at least one line in the `## Unreleased` block (line range from `grep -n '^## Unreleased' CHANGELOG.md` to the next `^## ` heading).
