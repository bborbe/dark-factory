...
- [ ] On `dark-factory daemon` start with default config, slog line `healthcheck startup gate starting` appears BEFORE any prompt-poll / watcher log line — evidence: `dark-factory daemon` stderr lines, `grep -n 'healthcheck startup gate starting' run.log` returns a line earlier than the first `watching for prompts` / `polling` line.
...
- [ ] On healthcheck failure, daemon exits non-zero with a category-naming error message identifying which probe failed (e.g. `healthcheck failed: docker daemon unreachable`) — evidence: `dark-factory daemon; echo $?` prints non-zero; stderr matches `^healthcheck failed: .+$`.
...
- [ ] `preflightCommand` behavior is UNCHANGED — evidence: `git diff master -- pkg/runner/preflight*.go` shows no semantic changes to `Preflight*` functions; existing preflight unit tests pass unmodified (`go test ./pkg/runner -run TestPreflight` exits 0); `--skip-preflight` still works (existing tests cover this); `preflightInterval` cache unchanged.
...
- [ ] `docs/running.md` cross-references the new startup gate in its healthcheck section — evidence: `grep -n 'healthcheck startup gate\|daemon startup' docs/running.md` returns ≥1 line referencing the new behavior.
