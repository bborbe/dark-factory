...
1. New config field `prompts.validationCommand` (string, default: `make precommit`).
2. Dark-factory appends the validation command to the prompt content (alongside the existing completion report suffix) before passing it to the executor.
3. The injected text instructs the YOLO agent to run the validation command and use its exit code to determine the completion report status.
4. If `prompts.validationCommand` is empty string, no validation command is injected (opt-out).
5. The prompt's own `<verification>` section is still included — the agent sees both. The injected validation command takes precedence for determining success/failure.
...
|---------|-------------------|----------|
| Project has no `make precommit` target | Agent reports failure in completion report | Set `prompts.validationCommand` to a valid command or empty string |
...
- [ ] `prompts.validationCommand` config field exists, defaults to `make precommit`
